      ...
      <div class="inner">
        <div class="buttons">
          <div class="control onoff">
            <button class="hide-text" onclick={() => toggleSpace(false)}
              >REMOTE OFF</button
            >
            <span>SPACE OFF</span>
          </div>
          ...
          <div class="control mute">
            <button class="hide-text" onclick={toggleMute}>MUTE</button>
            <span>MUTE</span>
          </div>

          <div class="control ch up">
            <button onclick={incrementChannel}>▲</button>
          </div>
          ...
          <div class="numbers">
            {#each { length: 9 } as _, i}
              <div class="control number">
                <button onclick={() => gotoChannel(i + 1)}>{i + 1}</button>
              </div>
            {/each}
            <div class="control number">
              <button onclick={() => gotoChannel(0)}>0</button>
            </div>
            <div class="control showhide">
              <button class="showhide" onclick={toggleContent}>
                SHOW/HIDE
              </button>
            </div>
          </div>
...
<style>
  .perspective {
    ...
    :global(body:not([data-space])) & {
      display: none;
    }

    :global(body[data-space]) & {
  ...
  .wrapper {
    ...
    :global(body[data-space='floating']) & {
  ...
  .remote {
    ...
    background-image:
      var(--plastic-texture-img, none),
      linear-gradient(to bottom, #d8d8d8 0%, #a5a5a5 74%);
    box-shadow:
      inset 0 14px 6px 0px #e0e0e0,
      inset 0 8px 10px 0px #252424,
  ...
  .inner {
    ...
    box-shadow:
      ...
      -20px 30px 0 0 #000,
      20px -30px 0 0 #000,
      -22px -32px 0px 0 #696464,
      -22px 32px 0px 0 #696464,
      22px -32px 0px 0 #696464,
      22px 32px 0px 0 #696464,
      inset 0px 0 6px 2px #646464,
      inset 0px 0 0 1px #000;
    background-image:
